...
let embed = new Discord.RichEmbed()
        .setTitle(":pushpin: Xazer | Help Command 1/2")
        .setColor("#ffe735")
        .addField("Vous êtes actuellement dans la page d'aide numéro 1 du bot : Xazer .", "Xazer est un bot discord Chill & Français . Je suis codés en JavaScript .\nPour tout problèmes , contacter `&\"Exocray#3979` .\n• Tapez `& + <commande>` pour utilisez la commande voulu .\n• Tapez `&help + <command>` pour afficher les informations sur la commande voulu .", true)
        .addBlankField()
...
client.on("message", function (message) {
  ...
  if (args[0].toLowerCase() === prefix + "help"){
let embed = new Discord.RichEmbed()
      .setTitle(":pushpin: Xazer | Help Command 2/2")
      .setColor("#ffe735")
      .addField("Vous êtes actuellement dans la page d'aide numéro 2 du bot : Xazer .", "Je rapelle que ce bot est codés en JavaScript par un amateur du code .", true)
      .addBlankField()
      ...
      .addBlankField()
      .setFooter("Xazer Bot   |   Francophone")
  message.channel.send(embed)
}
});
...
client.on('message', message => {
  ...
  let msg = ' SΔTUЯİS Bot fonctionnel <3'; // Le message envoyé .
...
client.on('message', function (message) {
  ...
  if (args[0].toLowerCase() === prefix + "createrole") {
    if (message.author.id !== '598563097359286275') return message.channel.send(`**__Ta pas l'autorisation pd__**, ${message.author}`)
